...
describe('HomeWork2', function () {
  ...
  describe('Deployment', function () {
    it('Check string', async () => {
      ...
      expect(await homeWork2.getStr()).to.equal(str);
    });
    it('Check owner', async () => {
      const { homeWork2, owner } = await loadFixture(deploy);
      expect(await homeWork2.owner()).to.equal(owner.address);
    });
  });
  ...
  describe('sendEther', function () {
    describe('sendEther', () => {
      it('sendEther', async () => {
        ...
        await expect(homeWork2.sendEther(user.address, value, { value: eth })).to.changeEtherBalances(
  ...
  describe('SetNumber1', function () {
    describe('Requires', function () {
      it('Check owner', async function () {
        ...
        const number = 200;
        await expect(homeWork2.connect(otherAccount).setNumber1(number)).to.be.revertedWith('You are not owner');
      });
    });
    describe('SetNumber1', function () {
      it('Check number', async () => {
        ...
        const number = 200;
        const tx = await homeWork2.setNumber1(number);
        await tx.wait();
        expect(await homeWork2.getNumber()).to.equal(number);
      });
    });
  });
  ...
  describe('SetNumber2', function () {
    describe('SetNumber2', function () {
      it('Check number', async () => {
        const { homeWork2 } = await loadFixture(deploy);
        const number = 200;
        const tx = await homeWork2.setNumber2(number);
        await tx.wait();
        expect(await homeWork2.getNumber()).to.equal(number);
      });
  ...
  describe('Devision', function () {
    describe('Requires', function () {
      it('Devision by zero', async () => {
        const { homeWork2 } = await loadFixture(deploy);
        const [a, b] = [1, 0];
        await expect(homeWork2.division(a, b)).to.revertedWithPanic(PANIC_CODES.DIVISION_BY_ZERO);
      });
    });
  ...
  describe('AddPayament', function () {
    describe('Require', function () {
      it('Check sendPayament', async () => {
        ...
        await expect(homeWork2.addPayment(user.address, { value: eth })).to.revertedWith(
    ...
    describe('Check SendPayament', function () {
      it('Check change balance', async () => {
        const { homeWork2, owner, user } = await loadFixture(deploy);
        const eth = ethers.utils.parseUnits('1.0', 'ether');
        await expect(homeWork2.addPayment(user.address, { value: eth })).to.changeEtherBalances(
  ...
  describe('SendPayament', function () {
    describe('Require', function () {
      it('Check sendPayament', async () => {
        const { homeWork2, owner, user } = await loadFixture(deploy);
        await expect(homeWork2.sendPayment(owner.address)).to.revertedWith('There are no payments for you');
      });
    });
    describe('SendPayament', function () {
      it('Check change balance', async () => {
        ...
        const tx = await homeWork2.addPayment(user.address, { value: eth });
        await tx.wait();
        await expect(homeWork2.connect(user).sendPayment(owner.address)).to.changeEtherBalances(
